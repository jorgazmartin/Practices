...
PC1PC2_plot_w_kmeans_clusters=ggplot(datos)+
  geom_point(aes(x=PC1,y=PC2,color=cluster,fill=grouping_factor),shape=21,size=3,stroke=1.5)+
...
PC1PC3_plot_w_kmeans_clusters=ggplot(datos)+
  geom_point(aes(x=PC1,y=PC3,color=cluster,fill=grouping_factor),shape=21,size=3,stroke=1.5)+
  scale_fill_manual(values=colors_base)+
  xlab(paste0("PC1: ",round(100*sum_pca$PC1[2],digits=2),"% variance"))+
...
pl_tsne_w_clusters=ggplot(datosb)+geom_point(aes(x=Tsne_1,y=Tsne_2,color=cluster,fill=grouping_factor),shape=21,size=3,stroke=1.5)+
...
pl_umap_w_clusters=ggplot(datos_umap)+geom_point(aes(x=umap_1,y=umap_2,color=cluster,fill=grouping_factor),shape=21,size=3,stroke=1.5)+
